...
export default function Login() {
  ...
  return (
    <div className="glass-panel animate-fade-in-up" style={{ width: '380px', maxWidth: '100%' }}>
      ...
      <form onSubmit={handleLogin} className="animate-fade-in-up delay-300">
        ...
        <div style={{ marginBottom: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <label style={{ fontSize: '12px', fontWeight: 500, color: 'var(--tf-text-secondary)' }}>Password</label>
            <Link to="/forgot-password" style={{ fontSize: '12px', fontWeight: 500, color: 'var(--tf-accent)', textDecoration: 'none', transition: 'opacity 0.2s' }} onMouseOver={e => e.currentTarget.style.opacity='0.8'} onMouseOut={e => e.currentTarget.style.opacity='1'}>
              Forgot password?
            </Link>
          </div>
          <input 
        ...
        <button type="submit" disabled={loading} className="btn-modern primary" style={{ marginBottom: '20px' }}>
          {loading ? (
            <><div className="animate-spin" style={{width: '16px', height: '16px', border: '2px solid rgba(255,255,255,0.3)', borderTopColor: '#fff', borderRadius: '50%'}} /> Signing in...</>
          ) : 'Sign in'}
        </button>
      </form>
      ...
      <div className="animate-fade-in-up delay-500" style={{ textAlign: 'center', fontSize: '13px', color: 'var(--tf-text-secondary)' }}>
        No account? <Link to="/register" style={{ color: 'var(--tf-accent)', fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={e => e.currentTarget.style.color='var(--tf-accent-hover)'} onMouseOut={e => e.currentTarget.style.color='var(--tf-accent)'}>Create one</Link>
      </div>
    </div>
  );
}
